...
def traindata():
    labels = []
    trainfile = os.listdir("train_digital_txt/11/") # 获取手写体图片名
    num = len(trainfile)
    # 长度784列，每一行存储一个文件
    # 以一个数组存储所有训练数据，行：文件总数，列：784
    trainarr = np.zeros((num ,784))
    print("所有txt文本数量:",num) #  num：所有txt文本数量
    print("相对路径：train_digital_pictures/11/") # 转换数据目录
    for i in range(0,num):
        thisfname = trainfile[i]
        thislabel = seplabel(thisfname)
        labels.append(thislabel)  # 记录txt文本对应的数字编号
        trainarr[i,:] = datatoarray("train_digital_txt/11/%s"  %(thisfname))# 加载txt文本数据    
    num = 0
